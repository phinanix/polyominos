...
pub fn rotational_deduplicate(ominos: &Vec<FreePointList>) -> Vec<FreePointList> {
    ...
    for omino in ominos {
        if !out
            .iter()
            .any(|prev_omino| rotational_equivalence(prev_omino, omino))
        {
...
pub fn next_edge_to_cover(pts: &FreePointList) -> Option<Edge> {
    ...
    edges_to_cover
        .iter()
        .copied()
        .filter(|Edge(pt, _d)| !pts.contains(pt))
        .next()
}
    ...
    for (translated_omino, translation) in translated_ominos_and_translations {
        ...
        if let Some(merged_pts) = merge_pts(&pts, translated_omino) {
            ...
            if let None = next_edge_to_cover(&merged_pts) {
                ...
                let new_config = ConfigurationTranslation {
                    pts: merged_pts,
...
pub fn add_tr_children(
    ...
    fn extract_possible_pairs<'a>(
        ...
        perimeter.iter().filter_map(move |&(Edge(fp, d))| {
            if (d == dir_to_cover) {
                Some((
                    translate_a_to_b(omino, fp, pt_to_cover).0,
...
pub mod test {
    ...
    fn point_fiddling() {
        let pts =
            [(0, 0), (1, 3), (4, 4), (-3, 6), (3, -5), (20, 0)].map(|(x, y)| FreePoint { x, y });
        pts.map(|p| point_assert(p));
    }

    #[test]
    fn iter_perimeter_slow_is_same() {
        let pts: FreePointList = [(0, 0)]
    ...
    fn unarrangeable25() -> FreePointList {
        ...
        for pt in pts {
            let flips = [
                ...
                mirror_x_axis(pt),
                mirror_y_axis(pt),
                mirror_x_axis(mirror_y_axis(pt)),
            ];
            for flip in flips {
                out.insert(flip);
            }
